...
namespace CosmosDB.Services
{
    public class CosmosDbservice
    {
		...
		{
			ItemResponse<Curso> itemResponse = await this.Container.CreateItemAsync(
				curso, 
				new PartitionKey(curso.categoria),
		...
		public async Task AddNewItemSpAsync(Curso curso)
		{
			dynamic[] cursos = new dynamic[]
			{
				new
				{
					curso.id,
			...
			var result = await this.Container.Scripts.ExecuteStoredProcedureAsync<string>(
				"createItem", new PartitionKey(curso.categoria), new[] { cursos });
		}
	}
}
